...
OP_PUSH = ['push']
OP_POP = ['pop']
ALL_OP = OP_LOAD + OP_STORE + OP_MOVE + OP_ALU + OP_COMPARE + \
    OP_JUMP + OP_CALL + OP_RETURN + OP_BRANCH + OP_PUSH + OP_POP
...
def compiler(line):
    op = line[0]
    if (op.isdigit()):
        return compile_digit(line)
    elif (op in ALL_OP):
        if (op == 'load'):
            return compile_load(line)
        elif (op == 'store'):
            return compile_store(line)
        elif (op == 'mov'):
            return compile_move(line)
        elif (op in ['add', 'sub', 'mul', 'div', 'mod', 'and', 'or', 'xor', 'not', 'neg', 'shl', 'shr']):
            return compile_alu(line)
        elif (op == 'cmp'):
            return compile_compare(line)
        elif (op in ['jmp', 'jeq', 'jne', 'jlt', 'jle', 'jgt', 'jge']):
            return compile_jump(line)
        elif (op == 'call'):
            return compile_call(line)
        elif (op == 'ret'):
            return compile_return(line)
        elif (op in ['brn', 'beq', 'bne', 'blt', 'ble', 'bgt', 'bge']):
            return compile_branch(line)
        elif (op == 'push'):
            return compile_push(line)
        elif (op == 'pop'):
            return compile_pop(line)
